...
class ICP_B_Server:
    def __init__(self, port=55555, app_id="default_app", reliability=0, length=0, message_type=1):
        """
        初始化 ICPServer 类，绑定到指定端口。
        :param port: 服务器端口号，默认为 55555
        :param app_id: 应用标识符，默认为 "default_app"
        :param reliability: 数据包可靠性，范围 0-3，默认为 0
        :param length: 数据包总长度，范围 0-1500，默认为 0
        :param message_type: ICP 消息类型，范围 0-3，默认为 1
        """
        self.port = port
        self.app_id = app_id
        ...
        self.socket.connect(f"tcp://192.168.20.223:{self.port}")

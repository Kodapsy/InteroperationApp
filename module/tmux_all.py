...
os.system(f"tmux new-window -t {session_name} - step2")
os.system(f'tmux send-keys -t {session_name}:step2 "python3 ../test/HMI_test.py" C-m')
time.sleep(1)
...
for cmd in commands:
    os.system(f'tmux send-keys -t {session_name}:step3 "{cmd}" C-m')
    time.sleep(1)
